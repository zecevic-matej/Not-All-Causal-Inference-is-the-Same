...
for p in ps:
    ...
    cfg.dir_exp = os.path.join(cfg.dir_exp, datetime.now().strftime("%Y%m%d-%H%M%S"))
    cfg.dir_exp = os.path.join(cfg.dir_exp, p)
    ...
    for seed in cfg.seeds:
        ...
        if not cfg.load_model:
            ...
            dir_seed = os.path.join(cfg.dir_exp, f"seed-{seed}")
            ...
            for step in range(cfg.max_epochs): # epochs now
                ...
                for i in range(len(train_data)):
                    ...
                    datapoint = next(train_ds)
                    for V in ncm.V:
                        ncm.S[V].zero_grad()

                    pV_est = ncm.forward(datapoint, samples=cfg.samples, doX=torch.tensor([-1.]*cfg.samples).unsqueeze(1)) # -1 means no intervention, clean up!!
                    ...
                    pV_int_est = ncm.forward(datapoint, samples=cfg.samples, doX=torch.tensor([0.] * cfg.samples).unsqueeze(1))  # TODO: clean the doX interface
                    ...
                    if i % cfg.loss_running_window == 0:
                        t1 = time.time()
                        ...
                        if running_loss_tr < best_loss_tr:
        ...
        checkpoint = torch.load(os.path.join(dir_seed_models,best_model_name))
        ...
        plot_marginals(pred_marginals, gt_marginals, seed, save=(dir_seed, '-best') if cfg.save_viz or not cfg.load_model else None)
        print(f'Computed Marginals for L1 distribution.')

        # intervention
        pred_marginals = ncm.compute_marginals(samples=cfg.samples, doX=1.)
        plot_marginals(pred_marginals, compute_gt_marginals(meta, 'L2_doX1'), seed, save=(dir_seed, '-doX1') if cfg.save_viz or not cfg.load_model else None)
        print(f'Computed Marginals for do(X=1).')

        # intervention
        pred_marginals = ncm.compute_marginals(samples=cfg.samples, doX=0.)
        plot_marginals(pred_marginals, compute_gt_marginals(meta, 'L2_doX0'), seed, save=(dir_seed, '-doX0') if cfg.save_viz or not cfg.load_model else None)
